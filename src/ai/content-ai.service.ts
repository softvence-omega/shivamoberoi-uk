...
export class ContentAiService {
  ...
  async generateContent(prompt: string, url?: string): Promise<{ status: string; message: string; data: ContentResult }> {
    ...
    try {
      ...
      const generatedContent = await this.retryableAiCall(prompt, 700, 0.7);
      if (!generatedContent) {
      ...
      const uniqueUrl = url || `custom-prompt-${uuidv4()}`;
      ...
      this.logger.error(`Failed generating content for ${url || 'no URL'} and prompt: ${prompt.substring(0, 20)}...: ${err.message}`, err.stack);
