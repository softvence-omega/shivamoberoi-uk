...
export class ContentAiService {
  ...
  async generateContent(prompt: string): Promise<ContentResult> {
    const cacheKey = `content_generate_${prompt.substring(0, 20)}`;
    try {
      const cached = await this.cacheManager.get<ContentResult>(cacheKey);
      if (cached) {
        this.logger.debug(`Returning cached content for prompt: ${prompt.substring(0, 20)}...`);
        return cached;
      }
      ...
      const result = this.formatResult(uniqueUrl, generatedContent, analysis);
      await this.cacheManager.set(cacheKey, result, this.config.cacheTtl).catch(cacheError => {
        this.logger.warn(`Failed to cache result: ${cacheError.message}`);
      });
      return result;
    } catch (err) {
      this.logger.error(`Failed generating content for prompt: ${prompt.substring(0, 20)}...: ${err.message}`, err.stack);
  ...
  private async analyzeContent(content: string, keywords: string[]): Promise<ContentAnalysis> {
