...
export async function analyzeImage(imageUrl: string) {
  ...
  try {
    ...
    const name = imageUrl.split('/').pop() || imageUrl; // Derive name from URL

    await browser.close();
    return { fileSize, width, height, isBlurry, name };
  } catch (err) {
    console.error(`Error analyzing image ${imageUrl}: ${err.message}`);
    await browser.close();
    return { fileSize: 0, width: 0, height: 0, isBlurry: true, name: imageUrl }; // Include name in error case
  }
}